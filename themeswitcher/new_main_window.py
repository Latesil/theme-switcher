...
from Themeswitcher.theme_switcher_constants import \
    theme_switcher_constants as constants
...
class AppWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.day_terminal_combo.set_active_id(
            current_desktop.get_value("active-day-profile-terminal"))
        self.night_terminal_combo.set_active_id(
            current_desktop.get_value("active-night-profile-terminal"))
        ...
        if self.current_night_wallpaper != "":
            ...
            helper.set_wallpaper_to_box(self.night_wallpaper_event_box,
                                        self.current_night_wallpaper)
        ...
        # this is probably not a best solution but for now it check time on start according to
        # time section and change what is needed (and only if auto is on)
    ...
    def on__left_switch_state_set(self, widget, state):
        ...
        # set visibility of time section, is switch is set to true
        if state:
    ...
    def on__reset_themes_clicked(self, button):
        ...
        self.retrieve_theme(self._light_combo_box, self._light_tree_model, self.cur_light_theme,
                            True, "light")
        self.retrieve_theme(self._dark_combo_box, self._dark_tree_model, self.cur_dark_theme, True,
                            "dark")
    ...
    def on__about_button_clicked(self, button):
        ...
        about.set_comments(
            _("A global automated switcher for dark/light GTK theme during day/night and more."))
    ...
    # first it checks what widget triggered by user. It looks at the name of the widget and
    # behave accordingly. If auto-switch is on and time is fit to change it try to set a new
    # terminal profile. BTW it cannot do it instantly. Instead you should reopen any existing
    # terminals.
    ...
    def on_terminal_combo_changed(self, combobox):
        if combobox.props.name == "night_terminal_combobox":
            ...
            if is_auto:
                if self.time_for_night():
                    current_desktop.set_terminal_profile(
                        current_desktop.get_value("active-night-profile-terminal"))
    ...
    def retrieve_theme(self, box, model, current_theme, default=False, theme=None):
        # get model from the box (we populate them in init function, after a populate theme list
        # from the system)
        model = box.get_model()
        for row in model:
            ...
            if default:
                ...
                if theme == "dark":
                    # check all themes until we find default adwaita-dark theme, set it in the
                    # combobox and return
                ...
                # if we want to set a default light theme
                    ...
                    # check all themes until we find default adwaita theme, set it in the
                    # combobox and return
                    if row[0] == "Adwaita":
                        box.set_active_iter(row.iter)
                        return
            ...
            # we want to set non-default theme (save current theme to the combobox)
                ...
                # theme variable here is equal to None (default value), so we dont have to add it
                # to the function calling if we need to set current theme.
    ...
    def wallpaper_button_clicked(self, button):
        ...
        dialog = Gtk.FileChooserDialog(_("Choose a file"), None, Gtk.FileChooserAction.OPEN,
    ...
    def get_values_list(self):
        # this is a list of time values [day_hours, day_minutes, night_hours, night_minutes])
        ...
        # current values equals to 20. If Night time is set to 20:00 it is equals to 1200 values
        # (because 60 * 20 = 1200) 570 values is equal 09:30 because 6*9 = 540, and + 30 minutes
        # = 570.
